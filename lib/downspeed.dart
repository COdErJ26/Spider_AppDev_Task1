...
class DownSpeed extends StatefulWidget {
  @override
  State<DownSpeed> createState() => _DownState();
}

class _DownState extends State<DownSpeed> {
  ...
  int checking() {
    if (dropdownvalue1 == 'Km/hour' && dropdownvalue2 == 'metre/sec') {
      return 1;
  ...
  String dropdownvalue1 = 'Km/hour';
  String dropdownvalue2 = 'metre/sec';

  var items = ['Km/hour', 'metre/sec'];
              ...
              ElevatedButton(
                ...
                onPressed: (() {
                  ...
                  if (condition == 1) {
                    setState(() {
                      answer =
                          '${((double.parse(unitVal.text) / 3.6)).toStringAsFixed(3)}';
                    });
                  } else if (condition == 3) {
                    setState(() {
                      answer =
...
class convertSpeed extends StatefulWidget {
  const convertSpeed({Key? key}) : super(key: key);

  @override
  State<convertSpeed> createState() => _convertSpeedState();
}

class _convertSpeedState extends State<convertSpeed> {
  void _showToast(BuildContext context, String textToBeShown) {
    final scaffold = ScaffoldMessenger.of(context);
    scaffold.showSnackBar(
      SnackBar(
              ...
              onPressed: (() {
                ...
                if (condition == 1) {
                  if ((double.parse(unitVal1.text)).toStringAsFixed(1) ==
                      ((double.parse(unitVal2.text) * 3.6))
                          .toStringAsFixed(1)) {
                    ...
                    setState(() {
                      ...
                      textshown = 'wrong';
                      iscolor = 1;
                      textAns = (double.parse(unitVal1.text) / 3.6)
                          .toStringAsFixed(2);
                    });
                  }
                  ;
