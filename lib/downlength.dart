...
class DownLength extends StatefulWidget {
  @override
  State<DownLength> createState() => _DownState();
}

class _DownState extends State<DownLength> {
  ...
  int checking() {
    if (dropdownvalue1 == 'Miles' && dropdownvalue2 == 'Kilometer') {
      return 1;
  ...
  String dropdownvalue1 = 'Miles';
  String dropdownvalue2 = 'Kilometer';

  var items = ['Miles', 'Kilometer'];
                ...
                ElevatedButton(
                  ...
                  onPressed: (() {
                    ...
                    if (condition == 1) {
                      setState(() {
                        answer =
                            '${((double.parse(unitVal.text) / 0.621)).toStringAsFixed(3)}';
                      });
                    } else if (condition == 3) {
                      setState(() {
                        answer =
...
class convertlength extends StatefulWidget {
  const convertlength({Key? key}) : super(key: key);

  @override
  State<convertlength> createState() => _convertlengthState();
}

class _convertlengthState extends State<convertlength> {
  void _showToast(BuildContext context, String textToBeShown) {
    final scaffold = ScaffoldMessenger.of(context);
    scaffold.showSnackBar(
      SnackBar(
          ...
          onPressed: (() {
            ...
            if (condition == 1) {
              if ((double.parse(unitVal1.text)).toStringAsFixed(1) ==
                  ((double.parse(unitVal2.text) * 0.621)).toStringAsFixed(1)) {
                ...
                setState(() {
                  ...
                  iscolor = 1;
                  textAns =
                      (double.parse(unitVal1.text) / 0.621).toStringAsFixed(2);
                });
              }
              ;
            } else if (condition == 3) {
              if ((double.parse(unitVal1.text)).toStringAsFixed(1) ==
                ...
                setState(() {
                  ...
                  textshown = 'wrong';
                  textAns =
                      (double.parse(unitVal1.text) * 0.621).toStringAsFixed(1);
                });
              }
            } else if (condition == 2) {
              if (double.parse(unitVal1.text).toStringAsFixed(1) ==
